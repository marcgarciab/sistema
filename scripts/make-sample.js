...
people.forEach(([name, total], i) => {
  ...
  const n = Math.max(0, total - (i % 3 === 0 ? 2 : 1)); // resto = fidelidad
  for (let k = 0; k < n; k++) {
...
writeFileSync(new URL('./sample-data.json', import.meta.url), JSON.stringify(buildHallOfFame(clients, rows), null, 2));
